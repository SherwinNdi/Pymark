...
- **Obesity without BED group:** $n = 120-150$ (BMI $\geq$  age-/sex-adjusted 95th percentile; no BED or other threshold eating disorder)
...
5. **Extreme motion during fMRI:**
   ...
   - $\geq$ 30% of volumes with FD > 0.9 mm (ABCD standard thresholds).
...
- Trial structure: Cue (1 s)  \to  Anticipation/delay (2-3 s)  \to  Target (200-500 ms)  \to  Outcome (1 s)  \to  Feedback.
...
For a given ROI time series $S(t)$ over a trial ($t \in \{t_0, t_1, ..., t_L\}$):
...
3. **Iterate through timepoints** $t \in \{t_m, t_m+1, ..., t_L\}$:
   - If $S(t_{i+1}) > B(t_i)$: **generate a spike** at time $t_{i+1}$, and update baseline as:

$$
B(t_{i+1}) = \alpha \cdot S(t_{i+1}) + (1-\alpha) \cdot B(t_i)
$$

   - If $S(t_{i+1}) \leq B(t_i)$: **no spike**, and reset baseline as:

$$
B(t_{i+1}) = S(t_{i+1})
$$

Where $\alpha \in [0, 1]$ is a parameter controlling how quickly the baseline adapts to signal increases. Typical value: $\alpha = 0.5$ (Kasabov et al. 2017).
...
- $\alpha$ (adaptation rate):** Kasabov et al. (2017) used $\alpha =$ 0.5. Sensitivity analyses will test $\alpha \in$  {0.3, 0.5, 0.7} to assess robustness.
...
- **Distance threshold (D_thr):** Two neurons are connected if their 3D distance in the SNNc cube is \leq  D_thr (typically 5-10 mm in physical space, corresponding to ~5-10 neuron steps in the discretized cube). This creates short-range ("local") connections.
...
If Δt \leq  0 (presynaptic spike precedes postsynaptic):
   - **Δw_ij = A+  \cdot  exp(Δt / Ï„+)**
...
- **Ï„+:** Temporal window (~1 time unit = 1 TR $\approx$  2 seconds; slightly longer windows may be useful for fMRI).
...
| $Reward \to Control$ coupling strength | w_ij from NAcc/OFC to dlPFC/IFG | Weaker (â†“) | Reduced top-down input from reward |
| Control \to Reward coupling strength | w_ij from dlPFC/IFG to NAcc/OFC | Weaker (â†“) | Reduced inhibitory modulation of reward |
...
1. **Three-group ANOVA (exploratory):**
   ...
   - Adjust for multiple comparisons: Bonferroni correction across ~12 primary neuromarkers  \to  \alpha_corrected = 0.05 / 12 $\approx$  0.004.
...
| $Reward \to Control$ coupling | Disinhibition scale (BIS-15) | Negative ($r < -0.2$) | 0.05 |
...
**Multiple comparison correction:** Use false discovery rate (FDR) control at q < 0.05 across all correlation tests (e.g., ~20-30 primary correlations × 2-3 sensitivity analyses  \to  ~60-90 tests total).
...
3. **Alternative spike-encoding parameters:** Re-run SNN training with different TBR thresholds ($\alpha \in$  {0.3, 0.7}) and different connection distance thresholds (D_thr \in  {3, 7} mm); report whether neuromarker magnitudes and group differences are robust to parameter choices.
...
| **Neuromarker** | **BED (mean ± SD)** | **Obesity (mean ± SD)** | **Healthy (mean ± SD)** | **Test Statistic** | **p-value** | **Effect Size (Cohen's d or $\eta^2$)** | **Interpretation** |
|---|---|---|---|---|---|---|---|
| NAcc activation (D_i) | ... | ... | ... | t(XX) = X.XX | $p < 0.001$ | $\eta^2$ = 0.12 | BED > controls âœ“ |
| dlPFC activation (D_i) | ... | ... | ... | F(2,XXX) = X.XX | $p = 0.045$ | $\eta^2$ = 0.03 | BED < controls âœ“ (medium) |
| Temporal offset | ... | ... | ... | t(XX) = X.XX | $p = 0.002$ | $d = 0.45$ | BED > controls âœ“ |
| $Reward \to Control$ coupling | ... | ... | ... | t(XX) = X.XX | $p = 0.078$ | $d = 0.25$ | Trend, not significant |
...
2. **Follow-up (outcome) requirements:**
   ...
   - Retention to outcome assessment $\geq$ 70% (if retention < 70%, report attrition bias analyses).
...
- **Power:** With n~600-700 at risk and 1-3% incidence rate, expect 6-21 incident cases. This is small but sufficient for logistic regression with ~10-20 predictors (rule of thumb: $\geq$ 5 outcome events per predictor). To increase power, we will combine BED with "any high binge-eating severity" as a secondary outcome.

**Outcome 2: Binge-Eating Symptom Trajectory**
- Categorical (primary):
  - **Worsening:** Baseline binge frequency  \to  Outcome binge frequency increases by $\geq$ 1 episode/month (or moves into new symptom category).
  - **Stable:** Change < 1 episode/month, remains in same category.
  - **Improving:** Binge frequency decreases by $\geq$ 1 episode/month.
...
- Linear change: Baseline EDE-Q binge subscale score  \to  Outcome EDE-Q binge subscale.
...
|---|---|---|
| **AUC (Area Under ROC Curve)** | Discrimination: ability to rank-order risk; ranges 0-1. | $\geq$ 0.70 (acceptable); $\geq$ 0.80 (good) |
| **Sensitivity (True Positive Rate)** | Among those who develop BED, % correctly identified. | $\geq$ 0.60 (detect majority of new cases) |
| **Specificity (True Negative Rate)** | Among those who don't develop BED, % correctly identified. | $\geq$ 0.85 (minimize false alarms) |
...
| **Negative Predictive Value (NPV)** | Among those predicted not to develop, % actually don't. | $\geq$ 95% (reassurance value) |
| **Calibration (Hosmer-Lemeshow test)** | Do predicted probabilities match actual proportions? | $p > 0.05$ (well-calibrated model) |
| **RÂ² (continuous outcome)** | Variance in outcome explained by model. | $\geq$ 0.15-0.25 (modest effect; absolute effect size) |
...
- **For logistic regression (binary outcome):** Use likelihood ratio (LR) tests comparing nested models; report $\chi^2$ and p-value.
  - Example: $\chi^2$(df=5, LR_BI-SNN) = 12.4, $p = 0.03$  \to  BI-SNN features significantly improve fit.

- **For continuous outcome:** Use partial F-tests or AIC/BIC comparisons.
  - Example: $\Delta R^2$ (Model 2  \to  Model 3) = 0.08, $p = 0.02$  \to  BI-SNN features explain additional 8% of variance.
...
3. **Pre-registration of feature hierarchy:** Specify which features are **primary** (motivated by prior literature and Study 1 findings) vs. **exploratory** (novel features discovered during analysis). Test primary features at $\alpha =$ 0.05; exploratory features at $\alpha =$ 0.01 (more stringent correction for multiple testing).
...
1. **Alternative wave combinations:** Repeat prediction using different predictor/outcome waves (e.g., ages 13-14  \to  15-16, or baseline 9-10  \to  outcome 15-16 for maximum prediction horizon).
...
- Model 3 AUC = 0.72-0.80; statistically significant improvement over Model 2 ($\Delta R^2$ = 0.10-0.15, $p < 0.05$).
...
- Model 3 (BI-SNN) does not significantly improve ($\Delta R^2$ < 0.05, $p > 0.10$).
...
**Findings:**
- Model 1 (demographics + baseline BMI + baseline symptoms) achieves AUC $\geq$  0.70; Models 2 and 3 do not improve substantially (ΔAUC < 0.05).
...
- **Reward hypersensitivity + inhibitory control insufficiency**  \to  greater vulnerability to environmental food cues and weight gain.
...
3. **Pre-registration of power analysis:** Calculate minimum detectable effect sizes for each primary comparison (e.g., with n_BED = 70, $\alpha =$ 0.05, 80% power, detect Cohen's $d = 0.45$ for neuromarker differences). Report in methods; interpret confidence intervals alongside p-values.
...
1. **Stringent motion thresholds:** Use mean FD < 0.5 mm; \leq 30% volumes with FD > 0.9 mm (stricter than ABCD defaults, which are ~< 0.75 mm). Report how many participants excluded at each threshold.
...
4. BI-SNN architecture parameters (cube size, distance threshold, encoding \alpha, learning parameters).
